...
static void controls_interact_from_frame(const DG_XR_FRAME *frame,
    ...
    out->special=(allowed==DG_IA_LADDER_CONTEXT || allowed==DG_IA_BEYOND_CONTEXT ||
        allowed==DG_IA_LOCKER_CONTEXT || allowed==DG_IA_DOWNED_CONTEXT) ? allowed : 0;
    ...
    if (out->special==DG_IA_BEYOND_CONTEXT) {
        if (!(left->thumbstick_x>=-1.0f && left->thumbstick_x<=1.0f))
            out->valid=0;
        if (!left->squeeze_click && !right->squeeze_click)
            out->levels|=(left->thumbstick_x< -0.5f?DG_IA_PEEP_LEFT:0) |
                         (left->thumbstick_x>0.5f?DG_IA_PEEP_RIGHT:0);
    }
    if (out->ladder) {
